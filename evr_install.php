...
function evr_upgrade_tables()
{
    ...
    if ($wpdb->get_var("SHOW TABLES LIKE '$new_event_tbl'") != $new_event_tbl) {
        $wpdb->query("CREATE TABLE IF NOT EXISTS " . $new_event_tbl . " SELECT * FROM " .
            $old_event_tbl);
  ...
  $new_answer_tbl = $wpdb->prefix . "evr_answer";
    ...
    if ($wpdb->get_var("SHOW TABLES LIKE '$new_answer_tbl'") != $new_answer_tbl) {
        ...
        $wpdb->query("CREATE TABLE IF NOT EXISTS " . $new_answer_tbl . " SELECT * FROM " .
            $old_answer_tbl);
    ...
    if ($wpdb->get_var("SHOW TABLES LIKE '$new_cost_tbl'") != $new_cost_tbl) {
        ...
        //Get attendee
        $attendees = $wpdb->get_results("SELECT * FROM " . get_option('evr_attendee') .
            " ORDER by id");
        if ($attendees) {
            foreach ($attendees as $attendee) {
                ...
                $costs = $wpdb->get_results("SELECT * FROM " . get_option('evr_cost') .
  ...
  //Update shortcodes if previous version
    ...
    if ($wpdb->get_var("SHOW TABLES LIKE '$old_organization_tbl'") == $old_organization_tbl) {
        $ER_org_data = $wpdb->get_row($wpdb->prepare("SELECT * FROM " . $old_organization_tbl .
            " WHERE id='1'"), ARRAY_A) or die(mysql_error());
    ...
    wp_mail("activation@wordpresseventregister.com", "Upgrade Complete - " . $upgrade_version,
